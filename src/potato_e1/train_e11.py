...
from .e11_dataset import E11CandidateCacheDataset, collate_e11_candidate_batch
from .e11_losses import compute_e11_loss
from .e11_model import CandidateResidualTransformer
from .train import apply_overrides, move_batch, seed_everything


def build_e11_dataset(config: dict[str, Any], split: str) -> E11CandidateCacheDataset:
    data = config["data"]
    if not data.get("manifest") or not data.get("root"):
        raise ValueError("data.manifest and data.root must be configured")
    if int(data.get("physics_dim", 0)) != 0:
        raise ValueError("E1.1 excludes physical features; set data.physics_dim=0")
    return E11CandidateCacheDataset(
        ...
        pair_candidate_iou=0.05,
        base_conf=float(data.get("base_conf", 0.25)),
        base_nms_iou=float(data.get("base_nms_iou", 0.30)),
    )


def build_e11_model(config: dict[str, Any]) -> CandidateResidualTransformer:
    data = config["data"]
    model = config["model"]
    return CandidateResidualTransformer(
        appearance_dim=int(data.get("appearance_dim", 0)),
...
def run_e11_epoch(
    ...
    keys = ("loss", "quality", "score", "ranking", "rescue", "delta_reg")
    totals = {key: 0.0 for key in keys}
    sample_count = 0

    progress = tqdm(loader, leave=False, desc="train-e11" if training else "val-e11")
    for batch in progress:
        ...
        with torch.set_grad_enabled(training), torch.autocast(
            ...
            output = model(batch)
            losses = compute_e11_loss(output, batch, config["loss"])
...
def main() -> None:
    ...
    train_dataset = build_e11_dataset(config, config["data"].get("train_split", "train"))
    val_dataset = build_e11_dataset(config, config["data"].get("val_split", "val"))
    loader_kwargs = {
    ...
    for epoch in range(1, int(config["train"]["epochs"]) + 1):
        train_metrics = run_e11_epoch(model, train_loader, device, config, optimizer, scaler)
        with torch.no_grad():
            val_metrics = run_e11_epoch(model, val_loader, device, config, None, scaler)
        scheduler.step()
